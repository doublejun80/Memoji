...
> **보관된 과거 분석 문서입니다.** 아래의 `%APPDATA%` 경로, `portable.txt`, 자동 백업,
> 여러 VDI가 하나의 SQLite DB를 함께 사용한다는 설명은 현재 구현과 다르거나 안전하지
> 않습니다. 현재 동작과 배포 절차는 `VDI_DEPLOYMENT_GUIDE.md`를 기준으로 하세요.
> 현재 앱은 `MEMOJI_DATA_PATH` → 실행 파일 옆 `data` → OS 로컬 데이터 fallback 순으로
> 시작 시 한 경로를 고정하며, 공유 DB 동시 접근과 자동 백업은 지원하지 않습니다.
